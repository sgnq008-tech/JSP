...
<jsp:useBean id="dao" class="com.memberone.studentDAO" scope="page" />
...
<tr>
	<td align = "center">검색된 결과가 없습니다.</td><br>
</tr>
<%
		}else {
%>
<tr>
	<td align="center"><br>
	※ 검색후, 아래 우편번호를 클릭하면 자동으로 주소입력됩니다.
	</td>
</tr>
<%
...
<tr>
	<td>
	<a href="javascript:sendAddress('<%=tZipcode%>','<%=tSido%>','<%=tGungun%>','<%=tDong%>','<%=tRi%>','<%=tBunji%>')">
	<%=tZipcode%>&nbsp;<%=tSido%>&nbsp;<%=tGungun%>&nbsp;<%=tDong%>&nbsp;<%=tRi%>&nbsp;<%=tBunji%>
	</a><br>
